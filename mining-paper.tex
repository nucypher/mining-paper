...
As adoption of the \nucypher~network increases, payments for access control services (`fees') will account for an increasing proportion of the earnings made by network nodes. However, before adoption has reached a sufficient threshold, nodes must be incentivized and subsidized via a regular inflationary reward.
...
A inflationary reward model of this sort should have the following properties:
\begin{itemize}
...
Initially, inflation subsidizes staking, but payments for re-encryption services will generate the majority or all of the revenues earned by stakers in the long run.
...
The individual daily compensation rate for a staker looks as:
\begin{eqnarray}
...
and a staker waiting for their stake to unlock after time $T$ (Spindown).
...
\subsection{Take compensation and spindown}

When the node spins down, the staker doesn't extend the time for end of staking $t_1$,
...
Connection problems may also be downtime when the staker upgrades their node's software, which is an absolutely legitimate reason to be offline for a short time.
