...
\usepackage[breaklinks,pdftitle={NuCypher: Mining \& Token Economics}, pdfauthor={Michael Egorov},colorlinks,urlcolor=blue,citecolor=darkgreen,linkcolor=darkblue]{hyperref}
\graphicspath{{pdf/}}
...
\section{A word on token governance}

As a decentralized network, the parameters of staking economics
(as well as other network characteristics),
should ultimately be determined by a consensus of the network's tokenholders.
As described by Aragon~\cite{aragon} and 0x~\cite{zrx}, tokens backed by effective governance may benefit from the following properties:

\begin{itemize}
    \item Adaptability to changing market conditions;
    \item Resistance to commoditization due to forks or competing networks;
    \item More secure execution of protocol upgrades;
    \item More representative token distribution across network stakeholders;
\end{itemize}

However, due to the industry's immature understanding of decentralized governance,
we decline to implement full governance at the outset.
A better approach is to introduce governance incrementally over time,
to allow \kms~to benefit from the experience of other networks and emergent best practices,
such as liquid democracy~\cite{liquid}.
