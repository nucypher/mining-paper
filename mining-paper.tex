...
\begin{abstract}
    This paper describes mining mechanisms and token economics present in \kms~network. The paper covers inflation rates, mechanisms to incentivize staking over the long-term, and estimates of the number of tokens that will be earned by nodes running typical procedures. The functions dictating the economics are also tuned for efficient operation in a smart contract environment. Finally, a number of price volatility mitigation strategies are proposed.
\end{abstract}
...
{\textit{Note: the terms `miner' and `staker', and their derivatives, are used interchangeably throughout this paper.}}

As the \kms~developer base grows, payment for re-encryption services (`network fees') will account for an increasing proportion of earnings made by network nodes. 
However, before developer adoption has reached a sufficient threshold, miners who run the nodes necessary for network operation must be subsidized.
This can be achieved via an inflation schedule and revenue distribution framework, where miners are the exclusive beneficiaries. 

An idealized set-up for inflation revenue distribution  would have the following properties:
\begin{itemize}
    \item The revenue from inflation distributed to stakers is proportional to their most recent stake size;
    \item The amount of work provided to stakers (and, hence, fees accrued) is proportional to their most recent stake size;
    \item Stakers are incentivized (by a higher reward rate) to run nodes for longer periods;
    \item Periods of high inflation do not significantly depreciate the token price, thereby maintaining liquidity for new stakers;
    \item Stakers are strongly incentivized to stay online all the time, in order to maximize availability to developers requiring re-encryption services. 
  	\item Stakers are incentivized to 're-stake' inflation revenue in order to maximize their long-term earnings.
	\item Short-term staking is also encouraged and subsidized, albeit at a lower rate. 
\end{itemize}

This paper addresses each of these desired properties, calculates expected earnings, and devises optimal mining/staking strategies.
...
This would start unlocking the tokens which he stakes linearly, during the time $T$.
...
caIf we take $I^*=350\%$ (turnover point of ZCash price in an overall bullish market), the corresponding inflation $I$ will be $140\%$~APR.
...
Fortunately, the exponential is a solution of a differential equation where inflation is proportional to the amount of yet not mined tokens:
\begin{eqnarray}
...
It depends on how early you start (the earlier, the better), and for how long you would commit to provide re-encryption services.
